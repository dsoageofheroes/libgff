...
#include "gff/gff.h"
//#include "gff/gff-map.h"
//#include "gff/gff-xmi.h"
#include "gff/image.h"
//#include "gff/wizard.h"
...
//extern void gffmod_write_image(const char *base_path, const int gff_idx, const int type_id, const int res_id) {
extern int gffmod_write_image(const char *base_path, gff_file_t *gff, const int type_id, const int res_id) {
    ...
    if (type_id != GFF_BMP && type_id != GFF_ICON && type_id != GFF_PORT) { return EXIT_FAILURE; }

    if (!gff->pals) {
        return EXIT_FAILURE;
    }

    pal = gff->pals->palettes;
    ...
    if (chunk.length != gff_read_chunk(gff, &chunk, &(cimg->data), chunk.length)) {
        printf("Can't read bmp!\n");
        return EXIT_FAILURE;
    }
    ...
    for (int cframe = 0; cframe < cimg->frame_num; cframe++) {
        ...
        uint16_t w = gff_get_frame_width(gff, type_id, res_id, cframe);
        uint16_t h = gff_get_frame_height(gff, type_id, res_id, cframe);
