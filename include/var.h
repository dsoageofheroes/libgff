#ifndef GFF_GPL_VAR_H
#define GFF_GPL_VAR_H
#include <stdint.h>
#include "gpl.h"
#include "gpl-lua.h"
#include "status.h"
...
typedef struct gff_gpl_name2_s {
    uint16_t addr; // Address of DSL routine
    uint16_t file; // file # holding DSL routine
...
typedef struct gff_gpl_check_s {
    union {
        gff_gpl_box_t box_check;
        gff_gpl_tile_t tile_check;
        gff_gpl_name_t name_check;
        gff_gpl_name2_t name2_check;
    } data;
    uint8_t type;
    uint16_t next;
} gff_gpl_check_t;
...
typedef struct gff_gpl_control_s {
    int16_t destx;
    int16_t desty;
...
extern int32_t      gff_gpl_global_big_num;
extern int32_t     *gff_gpl_global_big_numptr;
extern uint16_t     gff_gpl_current_file;
extern uint16_t     gff_gpl_current_type;

extern gff_status_t gff_gpl_init_vars();
extern gff_status_t gff_gpl_cleanup_vars();
extern gff_status_t gff_gpl_set_data_ptr(unsigned char *start, unsigned char *cpos);
extern gff_status_t gff_gpl_push_data_ptr(unsigned char *data);

extern gff_status_t gff_gpl_get_data_start_ptr(unsigned char **ret);
extern gff_status_t gff_gpl_get_data_ptr(unsigned char **ret);
extern gff_status_t gff_gpl_pop_data_ptr(unsigned char **ret);

extern gff_status_t gff_gpl_peek_half_word(uint16_t *hw);
extern gff_status_t gff_gpl_preview_byte(uint8_t offset, uint16_t *d);
