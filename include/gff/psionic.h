...
typedef struct gff_psi_abilities_s {
...
typedef struct gff_psin_s {
    uint8_t types[7];
} gff_psin_t;

typedef struct gff_psionic_list_s {
    uint8_t psionics[34];
} gff_psionic_list_t;

#define gff_psst_t gff_psionic_list_t
...
#define MAX_SPELLS (WIZ_MAX + CLERIC_MAX - 2)
...
#define MAX_WIZARD_SPELL (114)

typedef struct gff_spst_s {
    //uint8_t spells[MAX_SPELLS / 8 + 1];
    //uint8_t spells[138];
    uint8_t spells[MAX_PLAYERS][WIZ_MAX / 8 + 1];
    //uint8_t spells[MAX_PLAYERS][MAX_WIZARD_SPELL / 8 + 1];
    //uint8_t spells[MAX_PLAYERS][MAX_SPELLS / 8 + 1];
    uint8_t buf[126];
} gff_spst_t;

#define gff_spell_list_t gff_spst_t
